<?php
    ...
    if($Operacion=="Consulta"){
        ...
        $consulta= "INSERT INTO compra_produ(id_Compra, fecha_Hora, num_Ticket, Total, id_Proveedor) VALUES 
        (NULL,'$date',$numTicket,$Total,$Proveedor)"; 
        $res= mysqli_query($conexion,$consulta);
        if($res==1){
            $consulta = "SELECT id_Compra FROM compra_produ WHERE id_Proveedor=$Proveedor AND num_Ticket=$numTicket";
            $res=mysqli_query($conexion,$consulta);
            $datos = mysqli_fetch_array($res);
            $id_Compra=$datos[0];
